...
import { DELETE_ACTIVITY_REQUEST,DELETE_ACTIVITY_SUCCESS,DELETE_ACTIVITY_FAILED } from "../constants/requestsConstants";
export const acceptRequest=(userid,ouid)=>async(dispatch)=>{
 try{
        dispatch({type:ACCEPT_REQUEST_REQUEST});
        const {data}=await axios({
            method:'POST',
            url:`http://localhost:6000/api/v1/requests/acceptRequest/${userid}`,
...
export const deleteRequest=(userid,ouid)=>async(dispatch)=>{
 try{
        ...
        const {data}=await axios({
            method:'DELETE',
            url:`http://localhost:6000/api/v1/requests/deleteRequest/${userid}`,
            withCredentials:true,
            data:{
                ouid:ouid,
            }
...
export const deleteActivity=(userid,ouid)=>async(dispatch)=>{
 try{
        ...
        const {data}=await axios({
            method:'DELETE',
            url:`http://localhost:6000/api/v1/activity/deleteActivity/${userid}`,
            withCredentials:true,
            data:{
                ouid:ouid,
            }
